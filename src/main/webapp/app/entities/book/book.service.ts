...
export class BookService {
  ...
  protected convertDateFromClient(book: IBook): IBook {
    const copy: IBook = Object.assign({}, book, {
      releaseDate: book.releaseDate && book.releaseDate.isValid() ? book.releaseDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((book: IBook) => {
        book.releaseDate = book.releaseDate ? moment(book.releaseDate) : undefined;
      });
    }
    return res;
  }
