...
export class AuthorService {
  ...
  protected convertDateFromClient(author: IAuthor): IAuthor {
    const copy: IAuthor = Object.assign({}, author, {
      dateOfBirth: author.dateOfBirth && author.dateOfBirth.isValid() ? author.dateOfBirth.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((author: IAuthor) => {
        author.dateOfBirth = author.dateOfBirth ? moment(author.dateOfBirth) : undefined;
      });
    }
    return res;
  }
